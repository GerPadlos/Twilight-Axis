...
/obj/item/ritechalk/attack_self(mob/living/user)
	...
	switch(runeselection)
		...
		if("Rune of Psydon's Voice")
			to_chat(user,span_cultsmall("I begin inscribing the Allfather's exalted symbol..."))
			if(do_after(user, 30, src))
				playsound(src, 'sound/foley/scribble.ogg', 40, TRUE)
				new /obj/structure/ritualcircle/psydon_exalted(step_turf)
